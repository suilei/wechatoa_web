/**
*此js包含以下js：
*jquery-jtemplates.js 、jquery.ptip.js 及分享逻辑js
*/
/**
 ...
 (function() {
	 ...
	 S.prototype = { 
		...
		bind_share_content: function(opts){
		    var $tpl = $('#'+this.shareTplId).clone();
			var _this = this;
			$tpl.find('a').bind('click',opts,function(event){
		...
		opts.triggerBy
			.removeData('qtip') 
			 .qtip({
				...
				style: {
				   classes: 'chinaso-ui-tooltip ui-tooltip-shadow'
				}
			 });
	  }
	},
  ...
  */
  _share_to:function (params){
		...
		if (des == 'sk139') {
			...
			//支持图片的参数，从url参数网站自动解析图片，生成的title也会自动生成到此url的链接
